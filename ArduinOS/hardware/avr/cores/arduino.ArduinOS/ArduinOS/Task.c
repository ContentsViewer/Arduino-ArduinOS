    ...
    TaskHandle *createdTask, PortStackType *stackBuffer)
{
    ...
    if (newTCB != NULL)
    {
        ...
        #if(PORT_STACK_GROWTH < 0)
        {
            // 確保したメモリブロックの最後尾に移動する.
            // これは, スタックの先頭アドレスが低くなるように積まれていくからである.
        ...
        #else
        {
            ...
            newTCB->endOfStack = newTCB->stack + (stackDepth - 1);
        }
        #endif
...
void TaskStartScheduler(void)
{
    ...
    */
    ...
    else
    {
        ...
        // because there was not enough ArduinOS heap to create the idle task
        // or the timer task.
    }
}
